module.exports = {
  ...
  exits: {
    success: {
      ...
      extendedDescription: `user login extended description`,
      responseType: "ok"
    },
    badCombo: {
    ...
    badCombo2: {
      description: `The provided email does not
            match any user in the database.`,
      responseType: "unauthorized"
      // ^This uses the custom `unauthorized` response located in `api/responses/unauthorized.js`.
      // To customize the generic "unauthorized" response across this entire app, change that file
  ...
  fn: async function(inputs) {
    ...
    const token = await sails.helpers.passwords
      ...
      .then(() => {
        ...
        sails.log.info(
          "Users record has found!\n" +
            "User is " +
            userRecord.fullName +
            "\n" +
            "Now generating his jwToken.\n" +
            "Token : " +
            token
        );
